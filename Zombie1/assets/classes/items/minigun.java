...
public class minigun extends Item {
    ...
    public void LeftClickUse(Zombie z, int targetX, int targetY, Graphics g, Entity usingEntity) {
        Random r = new Random();
            int i1 = r.nextInt(240);
            if (i1 > 120) {
                i1 = i1 * (-1);
            }
            int i2 = r.nextInt(240);
            if (i2 > 120) {
                i2 = i2 * (-1);
            }
            ...
            for (int a = 0; a <= z.projectile.length - 1; a++) {
                if (z.projectile[a] == null) {
                    ...
                    z.projectile[a] = new WorldAccessor().getProjectile(z, "beamprojectileMinigun");
    ...
    @Override
    public minigun CreateNew(){
        
        minigun i = new minigun();
        i.name = this.name;
