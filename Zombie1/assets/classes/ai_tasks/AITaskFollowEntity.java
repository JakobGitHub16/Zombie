...
public class AITaskFollowEntity extends AITaskBase {
    ...
    public AITaskFollowEntity(){
        super();
        
        previousX = 0;
        previousY = 0;
        this.duration = 1000;
        this.name = "AITaskFollowEntity";
    }
    ...
    public void run(Zombie z, Graphics g){
        ...
        if (((Mob)owningEntity).targetingEntities.size() != 0){
            
            if (owningEntity.immunityFrames == 0){
                Entity e = ((Mob)owningEntity).targetingEntities.get(0);
                int dist = getDistanceAndSightInBlocks(z, e.X, e.Y, this.owningEntity.X, this.owningEntity.Y);
                
                if (markers.size() == 0 && !inRange(z, e.X, e.Y, this.owningEntity.X, this.owningEntity.Y))this.cancel = true;
                
                if (markers.size() == 0){
                    
                    if (dist >= 0 && dist <= seekingRange)
                    this.markers = addMarker(e.X, e.Y);
                    else {
                        owningEntity.motions.get("movement")[0] = 0;
                        owningEntity.motions.get("movement")[1] = 0;
                    }
                }
                ...
                if (markers.size() > 0){
                    ...
                    if (canGo(z, e.X, e.Y, this.owningEntity.X, this.owningEntity.Y)
                            && canGo(z, e.X, e.Y, this.owningEntity.X + this.owningEntity.hitboxes.get(0).sizeX, this.owningEntity.Y)
                            && canGo(z, e.X, e.Y, this.owningEntity.X, this.owningEntity.Y + this.owningEntity.hitboxes.get(0).sizeY)
                            && canGo(z, e.X, e.Y, this.owningEntity.X + this.owningEntity.hitboxes.get(0).sizeX, this.owningEntity.Y + this.owningEntity.hitboxes.get(0).sizeY)
                            && dist >= 0 && dist <= seekingRange){
                        clearMarkers();
                        this.markers = addMarker(e.X, e.Y);
                    }
                    ...
                    if (this.owningEntity.Y >= markers.get(markers.size() - 1)[1] - 10 && this.owningEntity.Y < markers.get(markers.size() - 1)[1] + 10){
                        ...
                        if (markers.size() == 0) if (inRange(z, e.X, e.Y, this.owningEntity.X, this.owningEntity.Y))this.markers = addMarker(e.X, e.Y);
                    }
    ...
    public boolean inRange(Zombie z, double targetX, double targetY, double fromX, double fromY){
        WorldAccessor w = new WorldAccessor();
        ...
        if (length / 60 <= this.seekingRange) return true;
        return false;
    }
    
    public void checkMarkers(Zombie z){
        int size = markers.size();
        for (int a = 0; a <= size - 2; a++){
            if (canGo(z, markers.get(a)[0], markers.get(a)[1], this.owningEntity.X, this.owningEntity.Y)
                    && canGo(z, markers.get(a)[0], markers.get(a)[1], this.owningEntity.X + this.owningEntity.hitboxes.get(0).sizeX, this.owningEntity.Y)
                    && canGo(z, markers.get(a)[0], markers.get(a)[1], this.owningEntity.X, this.owningEntity.Y + this.owningEntity.hitboxes.get(0).sizeY)
                    && canGo(z, markers.get(a)[0], markers.get(a)[1], this.owningEntity.X + this.owningEntity.hitboxes.get(0).sizeX, this.owningEntity.Y + this.owningEntity.hitboxes.get(0).sizeX)
                    )
            {
    ...
    public boolean canExecute(Zombie z, Graphics g){    
        boolean b = super.canExecute(z, g);
        if (b){
            if (((Mob)owningEntity).targetingEntities.size() > 0)
            if (canGo(z, ((Mob)owningEntity).targetingEntities.get(0).X, ((Mob)owningEntity).targetingEntities.get(0).Y, this.owningEntity.X, this.owningEntity.Y) == false
                    || this.owningEntity.chunkLayer != ((Mob)this.owningEntity).targetingEntities.get(0).chunkLayer
                    || !this.owningEntity.onMap.equals(((Mob)this.owningEntity).targetingEntities.get(0).onMap)) return false;
    ...
    public void onLoad(Map<String, String> params){
        ...
        if (params.containsKey("seekingRange")) this.seekingRange = Integer.parseInt(params.get("seekingRange"));
    ...
    @Override
    public AITaskFollowEntity CreateNew(){
        
        AITaskFollowEntity a = new AITaskFollowEntity();
    ...
    public void OnCreateNew(AITaskFollowEntity newCreatedAiTask){
